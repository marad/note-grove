...
fun main() {
    singleWindowApplication {
        ...
        val cardA = NoteCardState(Buffer("title a", Paths.get(""), root, AnnotatedString(text)))
        val cardB = NoteCardState(Buffer("title b", Paths.get(""), root, AnnotatedString("Donec efficitur elementum sem ut tempor. Ut sit amet arcu sit amet dui fringilla tincidunt vel quis erat. In eget diam efficitur turpis pulvinar sollicitudin eget nec diam. Fusce non tincidunt diam. Pellentesque a mauris eu eros pretium consectetur eu sit amet velit. Suspendisse augue metus, euismod eu tincidunt sed, auctor sit amet nibh. Etiam sed metus sollicitudin, placerat dolor a, porta leo. Cras in volutpat erat.")))

        val sharedBuffer = Buffer("title c", Paths.get(""), root, AnnotatedString("shared content"))
        ...
        MaterialTheme {
            Column(Modifier.fillMaxSize().padding(16.dp)) {
                ...
                Button(
                    onClick = {
                        val card = NoteCardState(Buffer("generated", Paths.get(""), root, AnnotatedString(text)))
